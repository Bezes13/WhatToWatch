...
import com.movies.whattowatch.model.dataClasses.ProviderID
...
class FirebaseRepository {
    ...
    suspend fun addProvider(provider: Int) {
        try {
            auth.uid?.let { collection.document(it).collection(providerDocument).document(provider.toString()).set(
                ProviderID(provider)
            ).await() }
        } catch (e: Exception) {
            e.printStackTrace()
